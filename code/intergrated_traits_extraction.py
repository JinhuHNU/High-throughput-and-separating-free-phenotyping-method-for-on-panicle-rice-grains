...
for i, item in enumerate(img_list):
    ...
    grains = utils.filter_dst_cont(contours, 1000)
    for k, grain in enumerate(grains):
        area.append(cv2.contourArea(grain)*0.042333*0.042333)
        length.append(utils.Get_length_width(grain)[0])
        width.append(utils.Get_length_width(grain)[1])
        perimeter.append(cv2.arcLength(grain, closed=True)*0.042333)
        lw_ratio.append(utils.Get_length_width(grain)[0]/utils.Get_length_width(grain)[1])
        ap_ratio.append((cv2.contourArea(grain)*0.042333*0.042333)/(cv2.arcLength(grain, closed=True)*0.042333))
        circularity.append(4*np.pi*(cv2.contourArea(grain)*0.04233*0.04233)/np.power((cv2.arcLength(grain, closed=True)*0.042333), 2))
