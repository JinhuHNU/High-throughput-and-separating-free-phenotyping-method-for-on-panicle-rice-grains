...
img_dir = r"C:\Users\DELL\Desktop\data\gz"
for i in range(20):

    img_name = r"gz_{}_3.tif".format(i+1)
    ...
    for item in contours:
        ...
        if area >=1000:
            ...
            length.append(utils.Get_length_width(contours)[0])
            width.append(utils.Get_length_width(contours)[1])
            areas.append(area*0.042333*0.042333)
            perimeter.append(cv2.arcLength(item, closed=True)*0.042333)
            lw_ratio.append(utils.Get_length_width(contours)[0]/utils.Get_length_width(contours)[1])
